...
import { DollarSign, TrendingUp, Calendar, CheckCircle2, Shield, Flame, Briefcase, Globe, Bot } from "lucide-react";
    ...
    return (
      ...
      >
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          {idea.recession_resistance_score !== null && (
            <div className="text-right">
              <div className="text-xs text-muted-foreground flex items-center gap-1 justify-end">
                <Shield className="w-3 h-3" />
                Recession
              </div>
              <div className={cn("text-sm font-medium", getScoreColor(idea.recession_resistance_score))}>
                {idea.recession_resistance_score}%
              </div>
            </div>
          )}
          {idea.pain_point_severity !== null && (
            <div className="text-right">
              <div className="text-xs text-muted-foreground flex items-center gap-1 justify-end">
                <Flame className="w-3 h-3" />
                Pain Point
              </div>
              <div className={cn("text-sm font-medium", getPainPointColor(idea.pain_point_severity))}>
      ...
      <div className="flex flex-wrap gap-4 mt-3 pt-3 border-t border-border/50">
        {idea.recession_resistance_score !== null && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-muted-foreground">Recession:</span>
            <span className={cn("text-xs font-medium", getScoreColor(idea.recession_resistance_score))}>
              {idea.recession_resistance_score}%
            </span>
          </div>
        )}
        {idea.pain_point_severity !== null && (
          <div className="flex items-center gap-1.5">
            ...
            <span className={cn("text-xs font-medium", getPainPointColor(idea.pain_point_severity))}>
              {idea.pain_point_severity}/10
            </span>
          </div>
        )}
